...
Plugin URI: https://www.cminds.com/wordpress-plugins-library/adchanger/
...
class CMAdChanger {
	...
	public function __construct() {
		 ...
		 * Shared
		 */
		require_once CMAC_PLUGIN_DIR . '/package/cminds-free.php';
	...
	private static function setupConstants() {
		...
		if ( !defined( 'CMAC_LICENSE_NAME' ) ) {
			define( 'CMAC_LICENSE_NAME', 'Ad Changer Client' );
		}
		...
		if ( !defined( 'CMAC_RELEASE_NOTES' ) ) {
			define( 'CMAC_RELEASE_NOTES', 'https://www.cminds.com/wordpress-plugins-library/adchanger/#changelog' );
		}
	}
...
add_action( 'admin_init', 'cmacf_init' );

function cmacf_init() {
	
	global $wpdb;
	
	$table_name = $wpdb->prefix . CMAC_CAMPAIGNS_TABLE;
	
	$row = $wpdb->get_results("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '".$table_name."' AND column_name = 'meta'");
	if(empty($row)){
		$altersql = "ALTER TABLE `".$table_name."` ADD COLUMN `meta` text NULL DEFAULT ''";			
		$wpdb->query($altersql);
	}
